...
describe("ResourcesAvailability", () => {
    it("should reject non-string name", async () => {
        const res = await chai
            ...
            .send({ name: 123, contactInfo: "Fire", assistance: "MetService rescue team"});
    ...
    it("should filter ResourcesAvailability by contactInfo", async () => {
        const res = await chai.request(app).get("/api/v1/ResourcesAvailability?contactInfo=Fire");

        chai.expect(res.body.data[0].contactInfo).to.be.equal("Fire");
    });
